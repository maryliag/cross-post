...
const openModal = async (app, payload, author, link) => {
    const viewData = payloads.openModal({
        trigger_id: payload.trigger_id,
        user_id: payload.user.id,
        username: payload.user.username,
        metadata: payload.message["ts"] + "," + author + "," + payload.channel.id + "," + link,
    });
...
const threadMessage = async (client, view, author, thread_ts) => {
    const values = view.state.values;
